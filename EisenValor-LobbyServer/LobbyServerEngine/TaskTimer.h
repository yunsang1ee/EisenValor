...
namespace LobbyServerEngine {
	...
	struct TaskData {
		TaskData(std::weak_ptr<LobbyServerEngine::TaskQueue> _owner, std::shared_ptr<Task> _task)
			:owner{ _owner }, task{ _task }
		{
		}

		std::weak_ptr<LobbyServerEngine::TaskQueue>	owner;
		std::shared_ptr<LobbyServerEngine::Task>	task;
	};
	...
	class TaskTimer : public Singleton<TaskTimer> {
		SINGLETON(TaskTimer)
	public:
		void Reserve(const std::chrono::milliseconds ms, std::weak_ptr<LobbyServerEngine::TaskQueue> owner, std::shared_ptr<Task> task);
