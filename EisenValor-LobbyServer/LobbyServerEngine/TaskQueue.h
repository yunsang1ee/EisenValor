...
namespace LobbyServerEngine {
	class Task;
	class TaskQueue : public std::enable_shared_from_this<LobbyServerEngine::TaskQueue> {
	public:
		TaskQueue();
		virtual ~TaskQueue();
	...
	public:
		...
		{
			MANAGER(LobbyServerEngine::TaskTimer)->Reserve(ms, shared_from_this(), MakeTask(std::forward<Func>(func)));
		}

		template<typename T, typename Ret, typename... FArgs, typename... CallArgs>
		void ExecTimer(const std::chrono::milliseconds ms, Ret(T::* memFunc)(FArgs...), CallArgs&&... args)
		{
			MANAGER(LobbyServerEngine::TaskTimer)->Reserve(ms, shared_from_this(), MakeTask(memFunc, std::forward<CallArgs>(args)...));
		}
