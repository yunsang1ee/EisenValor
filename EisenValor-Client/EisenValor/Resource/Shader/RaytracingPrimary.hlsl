...
inline float HardShadow(float3 origin, float3 dir)
{
	...
	RayDesc ray = { origin, 0.001, dir, 10000.0 };
...
void ClosestHitMain(inout RayPayload payload, in BuiltInTriangleIntersectionAttributes attribs)
{
	...
	float3 worldNormal = normalize(mul(normalize(v0.normal * bary.x + v1.normal * bary.y + v2.normal * bary.z), (float3x3) inst.worldIT));
	...
	float shadow = HardShadow(WorldRayOrigin() + WorldRayDirection() * RayTCurrent() + worldNormal * 0.01, normalize(float3(0.5, 1.0, 0.3)));
	payload.color = albedo * (saturate(dot(worldNormal, normalize(float3(0.5, 1.0, 0.3)))) * shadow * 2.0 + 0.15);
}
