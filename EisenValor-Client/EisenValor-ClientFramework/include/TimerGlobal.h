...
public:
	...
	inline void SetTargetFPS(uint32_t targetFPS) noexcept final { m_targetDeltaTime = targetFPS == 0 ? 0 : 1.0f / targetFPS; }
	inline void SetFixedFPS(uint32_t fixedFPS) noexcept final { m_fixedDeltaTime = fixedFPS == 0 ? 0 : 1.0f / fixedFPS; }

	[[nodiscard]]
	inline float GetRuntime() const noexcept final { return m_runTime; }
	[[nodiscard]]
	inline float GetFPS() const noexcept final { return static_cast<float>(m_curFPS); }
