...
void DxRtPipelineState::CreateGlobalRootSignature(ID3D12Device5* device, bool enableRestirCandidateRoot)
{
	...
	// Range 2: ReSTIR PrimaryHitCurrent UAV (u2)
	ranges[2] = {D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1, 2, 0, D3D12_DESCRIPTOR_RANGE_FLAG_NONE, 0};
	// Range 3: ReSTIR ReservoirInitial UAV (u3)
	ranges[3] = {D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1, 3, 0, D3D12_DESCRIPTOR_RANGE_FLAG_NONE, 0};
	...
	versionedDesc.Desc_1_1.NumParameters = enableRestirCandidateRoot ? 11 : 8;
...
)
{
	...
	D3D12_RAYTRACING_SHADER_CONFIG shaderConfig = {maxPayloadSizeBytes, 2 * sizeof(float)};
	...
	D3D12_STATE_SUBOBJECT globalRootSigObj = {
		D3D12_STATE_SUBOBJECT_TYPE_GLOBAL_ROOT_SIGNATURE, m_globalRootSignature.GetAddressOf()
	};
	subobjects.push_back(globalRootSigObj);

	D3D12_STATE_OBJECT_DESC stateObjectDesc = {
		D3D12_STATE_OBJECT_TYPE_RAYTRACING_PIPELINE, (UINT)subobjects.size(), subobjects.data()
	};
