...
namespace ServerEngine {
	...
	public:
		...
		void ExecuteAfterTime(uint64 afterMS, TaskFuncCB&& func) noexcept
		{
			std::shared_ptr<Task> task = ObjectPool<Task>::MakeShared(std::move(func));
			MANAGER(ServerEngine::TaskTimer)->Reserve(afterMS, shared_from_this(), task);
		}

		template<typename T, typename Ret, typename... Args>
		void ExecuteAfterTime(uint64 afterMS, Ret(T::* memFunc)(Args...), Args... args) noexcept
		{
			std::shared_ptr<T> owner = std::static_pointer_cast<T>(shared_from_this());
