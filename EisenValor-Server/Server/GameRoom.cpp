...
void Server::Contents::GameRoom::JoinGameRoom(const std::shared_ptr<ClientSession>& clientSession) noexcept
{
	...
	// entry is not permitted if the room's maximum capacity is exceeded.
	else if(m_info.currentParticipants >= MANAGER(GameDataManager)->GetGameRoomData().maxParticipants) {
