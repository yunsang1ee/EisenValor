...
void Server::Contents::GameMatchManager::Init()
{
	for(int i = 0; i < 10; ++i) {
		const uint16 id = i + 1;
		auto match = ServerEngine::ObjectPool<GameWorld>::MakeShared(id);
		m_matches.insert(std::make_pair(id, match));
	}
}

std::shared_ptr<Server::Contents::GameWorld> Server::Contents::GameMatchManager::GetMatch(const uint16 id)
{
