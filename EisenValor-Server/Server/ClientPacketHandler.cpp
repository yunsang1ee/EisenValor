...
#pragma region WORLD_PACKETS
	...
	PacketHandlerFuncs[static_cast<uint16>(PACKET_TYPE::CS_CHANGE_PLAYER_STANCE_PKT)] = [](const std::shared_ptr<ServerEngine::Session>& session, const char* const buffer) -> bool { return HandlePacket<FB_TABLES::CS_CHANGE_GENERAL_STANCE_PACKET>(ClientPackets::Handle_CS_CHANGE_GENERAL_STANCE_PACKET, session, buffer); };
