...
bool GameServer::ClientPacketHandler::Handle_CS_ENTER_GAME_WORLD_PACKET(const std::shared_ptr<GameServerEngine::PacketSession>& session, const FB_TABLES::CS_ENTER_GAME_WORLD_PACKET& recvPkt)
{
	...
	if(world) {
		...
		if(nullptr == lobbyServerSession) {
			LOG_WARNING("Rejected game world enter. Lobby server session is disconnected. UserID:{}, WorldID:{}", lobbySessionID, worldID);
			session->Disconnect("Lobby server disconnected");
			return true;
		}
